/////////////////////////////////////////
// Author - Siddharth Mark Joseph      //
// Controller - CurrentWeather.js      //
// Version - 1.0                       //
// Purpose - Hits the REST end point   //
// corresponding to the current weather//
// information.                        //
// Services Used - getid, $http        //
////////////////////////////////////////
(function() {

    'use strict';

    angular
        .module('weatherApp')
        .controller('CurrentWeather',CurrentWeather)

     function CurrentWeather(getid,$http)
     {
         var vm = this;
         vm.countries=[];
         vm.error=false;
         vm.target="";
         vm.weather;
         vm.toggle=true;
         vm.onSubmit = function()
         {

             getid.getData().then(function (results) {
                 vm.countries = results;
                 for(var i=0;i< vm.countries.length;i++)
                 {
                     if(vm.countries[i].name.toLowerCase()===vm.name.toLowerCase())
                     {
                         vm.target=vm.countries[i]._id;
                         break;
                     }

                 }
                 getRequest();
             });

         }
         function getRequest()
         {
             var url = "http://api.openweathermap.org/data/2.5/weather?id="+vm.target+"&APPID=ddb46fe87a2dfe703ce1ede0340b7cce";
             $http.get(url).then(function(results){
                 if(results["data"].cod=="404")
                 {
                     vm.error=true;
                     return;
                 }
                 vm.toggle=false;
                 vm.weather=results["data"].main;
             },function(error){
                 console.log(error);
             })
         }
         vm.set = function()
         {
             vm.toggle=true;
             vm.target="";
             vm.error=false;

         }



     }
})();